...
trait StreamingLineHandler: Unpin {
    type Error;
    type Output;
    ...
    ) -> core::task::Poll<Result<Self::Line<'a>, Self::NextLineError>>
    where
        Self: 'a;
    ...
    fn handle(
        ...
        cx: &mut core::task::Context<'_>,
    ) -> core::task::Poll<Result<Self::Output, Self::Error>> {
        ...
        let (o, line_len): (Self::FnOut, usize) = {
            let line: Self::Line<'_> = match Self::next_line(reader_pinned, cx) {
                ...
                core::task::Poll::Ready(Ok(line)) => line,
            };
        ...
        match Self::finalize(reader, line_len, o) {
            ...
            Ok(o) => core::task::Poll::Ready(Ok(o)),
        }
    }
}
